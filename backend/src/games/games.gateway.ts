...
export class GamesGateway {
	...
	handleJoinGame(@MessageBody() body: { userId: number; code: string }, @ConnectedSocket() client: Socket) {
		const game = this.gamesService.joinGame(
			body.code,
			body.userId,
			client.id,
		);
	
		client.join(body.code);
	
		this.server.to(body.code).emit('playerJoined', {
			userId: body.userId,
		});
	...
	handleStartGame(@MessageBody() body: { userId: number; code: string }) {
		const game = this.gamesService.startGame(
			body.code,
			body.userId,
		);
	...
	handleNext(@MessageBody() body: { code: string }) {
		...
		if (next === 'END') {
			const leaderboard =
			this.gamesService.getLeaderboard(body.code);
